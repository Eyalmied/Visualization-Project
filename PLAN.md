...
**Implementation status**: the full app is built and working — `app.R`, all of `R/` (`data_prep.R`, `city_coords.R`, `theme.R`, `helpers_traveler.R`, `helpers_airline.R`, `ui.R`, `server.R`), `www/custom.css`, `README.md`, and the `report/` scaffold (`report.qmd`, `ai_prompt_appendix.md`) are committed. Verified headlessly: all 9 chart functions run correctly on real filtered data, the full `shiny::runApp()` boots and serves both tabs, and `shiny::testServer()` confirms filters/KPI tiles/metric toggles/log-scale all update reactively as expected. Three real bugs were found and fixed during verification: (1) plotly's `pointNumber` resets per color-trace, so the scatter's linked-brushing selection needed a stable `key` aesthetic instead; (2) `event_data()` on the heatmap click and scatter lasso-select would never have fired without explicit `event_register()` calls; (3) the duration slider's default range used `round()`, which rounded the true max down and silently excluded boundary rows — fixed with `floor()`/`ceiling()`. Also fixed: `geom_smooth(method="loess")` on the full 300k-row dataset was a performance trap (LOESS scales very poorly with n) — the booking-curve chart now aggregates to mean price per `days_left` first; the scatter plot now caps rendering at 5,000 sampled points since plotly can't handle 300k interactive points in a browser. Note: an R Shiny convention bit us during setup — Shiny auto-sources every `.R` file in `R/` before running `app.R` unless a marker file `R/_disable_autoload.R` is present (it is, here), which is required because `ui.R`/`server.R` depend on `flights_df` being built first inside `app.R`. **Not yet verified**: an actual click-through in a real browser (only headless/programmatic checks were possible in this environment) — before the team's first full run-through, open the app with `shiny::runApp()` and manually exercise every filter, the reset buttons, and both linked-brushing interactions per the Verification section below.
